#!/bin/bash
# Si + NaCl re-benchmark matrix into runs/{sys}-SUFFIX-s{seed}.
# Same protocol as run_local_matrix.sh; never touches v1.0.1 artifacts.
SUFFIX=${1:?usage: run_local_matrix_tagged.sh SUFFIX (e.g. d1)}
set -uo pipefail
...
run_one () {
  ...
  local wd=runs/${sys}-${SUFFIX}-s${seed}
