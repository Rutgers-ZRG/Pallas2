...
TRUE 15 GPa run: first benchmark after the pressure fix (pressure now
reaches FrechetCellFilter + dimer external_stress; enthalpy = E + pV in
eV/A^3). Prior '15 GPa' runs (drag 4.483 eV; no-drag job 5754901) were
effectively P~0 and remain comparable only to each other.
...
print("=" * 60)
print("TRUE 15 GPa no-drag run(): Graphite -> HD (16 atoms)")
print("=" * 60)
...
config = PallasConfig(
    znucl=[6],
    fpcutoff=5.5, natx=200, pressure_gpa=press_gpa,
    n_probes=5,
    max_gen=20,
    patience=5,
...
print(f"Pressure: {press_gpa} GPa ({config.press:.5f} eV/A^3)")
