#!/usr/bin/env python3
"""Test B: FIRE optimizer + larger perturbation (0.2)."""
import os, sys, time
...
from pallas.optimize import cal_saddle as _orig_cal_saddle
import pallas.optimize as zfunc

# Monkey-patch cal_saddle to use FIRE optimizer
_real_cal_saddle = zfunc.cal_saddle
def cal_saddle_fire(*args, **kwargs):
    kwargs['optimizer'] = 'fire'
    return _real_cal_saddle(*args, **kwargs)
zfunc.cal_saddle = cal_saddle_fire
# Also patch the import in pallas
import pallas.core as pallas_mod
pallas_mod.cal_saddle = cal_saddle_fire
...
print("=" * 60)
print("Test B: FIRE + fp_step_scale=0.2 (8 atoms)")
print("=" * 60)
...
pallas_inst = Pallas(config)
pallas_inst.init_run(['POSCAR1', 'POSCAR2'])
G = pallas_inst.run_fp_guided(n_probes=3)
t1 = time.time()
print(f"\nPhase 1: {t1-t0:.0f}s")

print("\n>>> Barrier refinement")
path, bn = pallas_inst.refine_barrier()
t2 = time.time()
print(f"Phase 2: {t2-t1:.0f}s")

print("\n>>> Validation")
stats = pallas_inst.validate_graph()
t3 = time.time()

print(f"\n>>> FINAL")
try:
    path, bn = pallas_inst.find_best_path()
